...
# Copy entrypoint first and make executable (DO THIS WHILE STILL ROOT)
...
# Create an unprivileged user, chown project, then switch to that user
RUN adduser --disabled-password --gecos "" appuser \
    && chown -R appuser:appuser /app

USER appuser

# Runtime env defaults (can be overridden by Render)
ENV PORT=8000
